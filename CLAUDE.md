...
- `threshold_tunning.py`: threshold search and calibration helpers, added upstream in `fe5e9a0`. `evaluate.py` only uses `apply_calibration` from it, in the PaDiM-NF branch (`evaluate.py:300`) — the rest (`tune_thresholds_on_validation`, `best_seg_threshold_f1`, …) is unused by the DRAEM path and must stay that way, see **Thresholds**.
